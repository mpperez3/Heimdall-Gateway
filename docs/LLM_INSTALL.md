# LLM Server - Guia de Instalacion para LLMs (Agentes)

> **Audiencia:** Este documento es para **LLMs / agentes de codificacion** que deben instalar y configurar LLM Server en una maquina del usuario de forma autonoma o semi-asistida. Si eres humano, usa `README.md#Install`.
...
*   **Verifica siempre.** Despues de cada paso corre `llm-server info` y `curl /v1/models`.
...
llm-server info 2>&1 | head -n 80
ls -la ~/.config/llm-server/conf.json /etc/llm-server/conf.json 2>&1 | head
ls -la ~/.local/state/llm-server/catalog.json /var/lib/llm-server/catalog.json 2>&1 | head
cat ~/.config/llm-server/llm-server.env 2>&1 | head -n 30
```

Si `llm-server` no esta en `PATH`, el binario esta en `~/.local/bin/llm-server` tras `pip install .` o disponible via `./llamacpp_stack/bundle/install_llamacpp_stack.sh`.
...
> "¿Quieres instalacion **user** (sin sudo, todo en `~/.local/`, `~/.config/llm-server/`, `~/.local/state/llm-server/`) o **system** (requiere `sudo`, en `/etc/llm-server`, `/var/lib/llm-server`, servicios del sistema)?"
...
> "¿Donde guardo los modelos? Ej: `/var/llamacpp_models` (default user: `~/.local/share/llm-server/models` o `/var/llamacpp_models` si existe). ¿Cuanto espacio tienes? Un Qwen 32B Q4 ~18GB."
...
*   Flags: `--llama-cpp-mode source|prebuilt|native` + opcional `--llama-cpp-ref <tag|commit>` (fuerza `source`). Si eliges `commit`, pregunta: "¿Que ref? Ej: `b6408`, `master`." -> `LLM_SERVER_LLAMA_CPP_REF`.

### Q5 - Red / puertos
> "¿Expongo en `127.0.0.1` (solo local) o `0.0.0.0` (LAN)? Puertos: por defecto `ollama_port+2` para llama-swap (`11436` si Ollama en `11434`) y `ollama_port+1` para API LLM Server (`11435`). ¿Tienes Ollama en `11434`? ¿Quieres puertos custom?"
...
> "¿Activo auth en el API LLM Server (`:11435`)? Si sí, ¿me das `api-key` o genero una? ¿Activo HTTPS con cert autofirmado? Si tienes cert propio, dame rutas `cert_file`/`key_file`."
...
*   Post-install: `llm-server add -hf <repo>:<quant>` y `llm-server run -hf <repo>:<quant> --auto`
...
*   Flags: `--idle-ttl 300`, env `LLM_SERVER_DISABLE_AGGRESSIVE_CUDA=1` para desactivar, `--prefer-source-cuda/--no-prefer-source-cuda`, `--prefer-binary/--no-prefer-binary`.
...
| `llm-server info` muestra instalacion existente | Pregunta `full` vs `package-only` (ver `prompt_existing_install_action` en `install.py:775`); usa `--update-binaries/--no-update-binaries` segun respuesta |
...
llm-server install --mode user --backend auto --models-dir /var/llamacpp_models --idle-ttl 300 --dry-run
# revisa el plan, luego sin --dry-run
llm-server install --mode user --backend auto --models-dir /var/llamacpp_models --idle-ttl 300
...
sudo llm-server install --mode system --backend auto --models-dir /var/llamacpp_models --public-host 0.0.0.0 --public-port 11436
...
llm-server install --mode user --backend auto --models-dir /var/llamacpp_models \
  --api-auth --api-https --public-host 0.0.0.0 --idle-ttl 300

# Forzar ref concreto de llama.cpp
llm-server install --mode user --backend auto --llama-cpp-mode source --llama-cpp-ref b10156 --models-dir /var/llamacpp_models
```
...
llm-server install --mode user --backend auto --no-update-binaries --package-only-update
```
...
```bash
# 5.1 Servicios
systemctl --user status llm-server-manager llm-server-router  # user mode
# o sudo systemctl status llm-server-manager llm-server-router  # system mode
...
llm-server config-keys --format json | jq '.logging.requests_log'
...
journalctl --user -u llm-server-manager -n 100 --no-pager
journalctl --user -u llm-server-router -n 100 --no-pager

# Rotación: api-requests.log.YYYY-MM-DD[.partN] (split por max_bytes, prune >retain_days=3, symlink api-requests.log → día activo)
# Env vars: LLM_SERVER_REQUESTS_LOG_PATH > conf.json logging.requests_log.path > DEFAULT; MAX_BYTES/RETAIN_DAYS vía env o conf.json (clamp 65536-1GiB / 1-30)
...
Si `API status: not reachable`, comprueba `LLM_SERVER_PUBLIC_HOST`/`PORT` en `~/.config/llm-server/llm-server.env` y `conf.json`.
...
Si difieren, `llm-server update <id> --auto` re-registra.
...
llm-server run -hf org/base-model:Q4_K_M --speculative -hf org/draft-model:IQ1_M
...
| Catalogo modelos | `~/.local/state/llm-server/catalog.json` | `/var/lib/llm-server/catalog.json` |
...
| Request log (rotado diario, split, 3 días) | `~/.local/state/llm-server/api-requests.log.YYYY-MM-DD[.partN]` + symlink `api-requests.log` → día activo; prune 3 días; fallback `/tmp/llm-server-api-requests.log.YYYY-MM-DD` | `/var/lib/llm-server/api-requests.log.YYYY-MM-DD[.partN]` |
...
*   **Sin `slot print_timing` en EXL3 (exllama/buun)/vLLM**: es esperado. Solo `llama-server` emite `slot print_timing`. Para **buun (EXL3 native)**, exllama legacy y vLLM el timing vive en `timings` (`prompt_ms`/`predicted_ms`/`draft_n`) y en `curl :11436/api/metrics/activity`. No reportes como bug si `llm-server logs --journal | grep timing` no muestra `print_timing` para esos backends. Buun: `timings.prompt_ms`/`predicted_ms` medidos, `draft_n`/`draft_n_accepted` solo con MTP.
...
- [ ] `llm-server info` muestra `reachable` en API y UI
- [ ] `curl /v1/models` responde `200` (aunque sea lista vacia)
- [ ] Si el usuario pidio modelo, `llm-server list` lo muestra y `curl /v1/chat/completions` con ese `model` devuelve `choices`
- [ ] `llm-server config-keys --format json | jq '.logging.llama_swap'` muestra `{"logToStdout":"both","logLevel":"info"}` o valor configurado (whitelist both/proxy/upstream/none)
...
- [ ] `llm-server logs --lines 200 --journal | grep timing` interpretado correctamente (EXL3/vLLM sin print_timing es esperado)
...
> Compatibilidad deprecada: instalaciones previas con `heimdall-gateway` migran automaticamente a `llm-server`. El alias `heimdall-gateway` delega en `llm-server` con aviso `WARNING: heimdall-gateway is deprecated, use llm-server` en stderr. Rutas `~/.config/heimdall-gateway`, `~/.local/state/heimdall-gateway`, `/etc/heimdall-gateway`, `/var/lib/heimdall-gateway`, `/opt/heimdall-gateway` se copian copy-if-missing a `~/.config/llm-server`, `~/.local/state/llm-server`, `/etc/llm-server`, `/var/lib/llm-server`, `/opt/llm-server` (backup `*.pre-llm-server-migration-*`, units viejas `heimdall-gateway-manager`/`router` deshabilitadas). Env `HEIMDALL_GATEWAY_*` aun aceptado como fallback de `LLM_SERVER_*` (migra deprecado). No edites `config.yaml` a mano.
...
*   `llamacpp_stack/cli.py: build_help_epilog`, `install --help`, `llm-server info --help`.
...
## LLM Server Autotune Skill

`skills/llm-server-autotune/SKILL.md` - Loop automático para optimizar `qwen3.8` etc. tras `add` o `optimizar`. Valida con `parallel 2/3` y guarda snapshot en `configs/history/`.
