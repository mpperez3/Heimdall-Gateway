...
# Raw ring buffer (T2b) - lazy import available as log_raw_request
try:
    from llamacpp_stack.cli.raw_log import log_raw_request  # type: ignore
except Exception:  # pragma: no cover
...
try:
    from llamacpp_stack.cli.gateway import (  # type: ignore
        DEDUP_STATE as _GW_DEDUP_STATE,
        _DEDUP_STREAM_TEES as _GW_DEDUP_TEES,
        _default_dedup_inflight_config as _GW_default_dedup_inflight_config,
        _normalize_dedup_inflight_config as _GW_normalize_dedup_inflight_config,
        _dedup_should_bypass as _GW_dedup_should_bypass,
        _dedup_extract_principal as _GW_dedup_extract_principal,
        _dedup_build_fingerprint as _GW_dedup_build_fingerprint,
        _dedup_send_cached_response as _GW_dedup_send_cached_response,
        _dedup_send_json_with_header as _GW_dedup_send_json_with_header,
        _dedup_streaming_finalize as _GW_dedup_streaming_finalize,
        _log_chat_stop_without_tools as _GW_log_chat_stop_without_tools,
        _tool_call_debug_summary as _GW_tool_call_debug_summary,
        resolve_chat_last_response_log_config as _GW_resolve_chat_last_response_log_config,
        _normalize_experimental_config as _GW_normalize_experimental_config,
        _default_experimental_config as _GW_default_experimental_config,
        run_manager_command as _GW_run_manager_command,
        manager_hint as _GW_manager_hint,
        get_public_endpoint_status as _GW_get_public_endpoint_status,
        infer_install_mode as _GW_infer_install_mode,
        service_commands_for_mode as _GW_service_commands_for_mode,
    )

    DEDUP_STATE = _GW_DEDUP_STATE
    _DEDUP_STREAM_TEES = _GW_DEDUP_TEES  # type: ignore
    _default_dedup_inflight_config = _GW_default_dedup_inflight_config  # type: ignore
    _normalize_dedup_inflight_config = _GW_normalize_dedup_inflight_config  # type: ignore
    _dedup_should_bypass = _GW_dedup_should_bypass  # type: ignore
    _dedup_extract_principal = _GW_dedup_extract_principal  # type: ignore
    _dedup_build_fingerprint = _GW_dedup_build_fingerprint  # type: ignore
    _dedup_send_cached_response = _GW_dedup_send_cached_response  # type: ignore
    _dedup_send_json_with_header = _GW_dedup_send_json_with_header  # type: ignore
    _dedup_streaming_finalize = _GW_dedup_streaming_finalize  # type: ignore
    _log_chat_stop_without_tools = _GW_log_chat_stop_without_tools  # type: ignore
    _tool_call_debug_summary = _GW_tool_call_debug_summary  # type: ignore
    resolve_chat_last_response_log_config = _GW_resolve_chat_last_response_log_config  # type: ignore
    _normalize_experimental_config = _GW_normalize_experimental_config  # type: ignore
    _default_experimental_config = _GW_default_experimental_config  # type: ignore
    run_manager_command = _GW_run_manager_command  # type: ignore
    manager_hint = _GW_manager_hint  # type: ignore
    get_public_endpoint_status = _GW_get_public_endpoint_status  # type: ignore
    infer_install_mode = _GW_infer_install_mode  # type: ignore
    service_commands_for_mode = _GW_service_commands_for_mode  # type: ignore
except Exception:
    pass
...
def _normalize_requests_log_config(raw: object) -> tuple[dict[str, object], bool]:
    ...
    if path_str != str(raw.get("path", "") or "").strip() if isinstance(raw.get("path", ""), str) else True:
        if path_str != raw.get("path"):
            changed = True
...
def _effective_requests_log_config() -> dict[str, object]:
    ...
    if env_retain.strip():
        try:
            ...
            cfg["retain_days"] = max(1, min(30, iv))
        except Exception:
            pass
    return cfg
...
def _daily_active_path_for(base: Path) -> Path:
    try:
        date_str = datetime.now().astimezone().date().isoformat()
    except Exception:
        date_str = datetime.now(timezone.utc).date().isoformat()
    return base.parent / f"{base.name}.{date_str}"


def _find_part_path(active: Path, line_len: int, max_bytes: int) -> Path:
    try:
        if not active.exists():
            return active
        sz = active.stat().st_size
        if sz + line_len <= max_bytes:
            return active
    except Exception:
        return active
    n = 1
    while True:
        candidate = Path(f"{active}.part{n}")
        try:
...
def _ensure_compat_symlink(base: Path, active: Path) -> None:
    try:
        if not active.exists():
            return
        link = base
        if link.exists() or link.is_symlink():
            try:
                if link.is_symlink():
                    target = link.readlink() if hasattr(link, "readlink") else Path(os.readlink(link))
                    if target == active or target == active.name or str(target) == str(active):
                        return
                    try:
                        link.unlink()
                    except Exception:
                    ...
                    if link.resolve() == active.resolve():
                        return
                    try:
                        link.unlink()
                    except Exception:
...
def _prune_old_logs(base: Path, retain_days: int) -> None:
    try:
        retain_days = max(1, min(30, int(retain_days)))
    except Exception:
        retain_days = 3
    try:
...
def normalize_server_config_payload(payload: dict[str, object]) -> tuple[dict[str, object], bool]:
    ...
    if not isinstance(raw_logging, dict):
        raw_logging = {}
        changed = True
    norm_logging: dict[str, object] = {}
    existing_logging_changed = False
    if isinstance(result.get("logging"), dict):
        norm_logging = dict(result.get("logging"))  # type: ignore
    else:
        existing_logging_changed = True
    raw_req = raw_logging.get("requests_log") if isinstance(raw_logging, dict) else None
...
def log_api_event(kind: str, payload: dict | None = None, log_path: Path | str | None = None) -> None:
    if payload is None:
        payload = {}
    entry = {
        "ts": datetime.now(timezone.utc).isoformat(),
    ...
    try:
        ...
        retain_days = int(cfg.get("retain_days", 3))
    except Exception:
        max_bytes = 10485760
        retain_days = 3
    max_bytes = max(65536, min(1073741824, max_bytes))
    retain_days = max(1, min(30, retain_days))
    if log_path is not None:
        try:
            primary_base = Path(log_path).expanduser()
        except Exception:
            primary_base = DEFAULT_REQUESTS_LOG_PATH
    else:
        primary_base = _resolve_requests_log_base_path(None)
    errors: list[str] = []

    def _try_write_base(base: Path) -> bool:
        try:
            base.parent.mkdir(parents=True, exist_ok=True)
        except Exception as exc:
            errors.append(f"{base}: mkdir {exc}")
            return False
        probe_existed = base.exists()
        try:
            with base.open("a", encoding="utf-8"):
                pass
            if not probe_existed and base.exists():
                try:
                    if not base.is_symlink() and base.stat().st_size == 0:
                        base.unlink()
                except Exception:
                    pass
        except Exception as exc:
            errors.append(f"{base}: probe {exc}")
            return False
        active = _daily_active_path_for(base)
        target = _find_part_path(active, line_bytes_len, max_bytes)
        try:
            ...
            return False
        try:
            _ensure_compat_symlink(base, active)
        except Exception:
            pass
        try:
            _prune_old_logs(base, retain_days)
        except Exception:
            pass
        return True

    if _try_write_base(primary_base):
        return
    if primary_base != fallback_base:
        if _try_write_base(fallback_base):
            return
...
def _extract_load_capabilities_from_value(value: object) -> list[str]:
    ...
    for candidate in candidates:
        ...
        for match in re.findall(r"(?:image-text-to-text|text-to-text|image-to-text|image|vision)", lowered):
            tokens.append("image-text-to-text" if match == "vision" else match)
...
def run_llamaswap_guard(args) -> int:
    ...
    class GuardHandler(BaseHTTPRequestHandler):
        ...
        def _proxy(self):
            ...
            try:
                ...
                _bundle_guard2 = None
                _bundle_ref_guard2 = None
                try:
                    import uuid as _uuid_guard2
                    ...
                    try:
                        _bundle_ref_guard2 = (
                            ...
                            or _uuid_guard2.uuid4().hex[:8]
                        )
                    except Exception:
                        _bundle_ref_guard2 = _uuid_guard2.uuid4().hex[:8]
                    try:
                        from llamacpp_stack.cli.crash_bundle import collect_engine_crash_bundle as _collect_guard2  # type: ignore
                    except Exception:
                        _collect_guard2 = None  # type: ignore
                    if _collect_guard2 is not None:
                        try:
                            _bundle_guard2 = _collect_guard2("llama-server", port=int(backend_port), pid=None, cmdline=None, returncode=None, timeout_s=8.0)
                        except Exception as _be:
                            _bundle_guard2 = {"engine": "llama-server", "journal_router_tail": f"unavailable: {type(_be).__name__}", "journal_manager_tail": f"unavailable: {type(_be).__name__}", "nvidia_smi": f"unavailable: {type(_be).__name__}", "probed_model": "", "pid": None, "port": int(backend_port), "cmdline": "", "returncode": None}
                    else:
                        _bundle_guard2 = {"engine": "llama-server", "journal_router_tail": "unavailable: ImportError", "journal_manager_tail": "unavailable: ImportError", "nvidia_smi": "unavailable: ImportError", "probed_model": "", "pid": None, "port": int(backend_port), "cmdline": "", "returncode": None}
                    try:
                        log_api_event("proxy_error_with_bundle", {"method": self.command, "path": self.path, "error": str(exc)[:1000], "bundle": _bundle_guard2, "bundle_ref": _bundle_ref_guard2})
                    except Exception:
                        pass
                except Exception:
                    pass
                try:
                    ...
                    self.send_header("Content-Length", str(len(data_g2)))
                    self.end_headers()
                    self.wfile.write(data_g2)
                    return
                except Exception:
                    pass
    ...
    def _watch_child():
        ...
        if not stop_event.is_set():
            ...
            try:
                from llamacpp_stack.cli.crash_bundle import collect_engine_crash_bundle as _collect_guard_child  # type: ignore

                _bundle_gc = _collect_guard_child("llama-swap", port=int(backend_port), pid=getattr(child, "pid", None), cmdline=None, returncode=rc, timeout_s=8.0)
            except Exception as _be:
                _bundle_gc = {"engine": "llama-swap", "pid": getattr(child, "pid", None), "port": int(backend_port), "cmdline": f"unavailable: {type(_be).__name__}", "returncode": rc, "journal_router_tail": f"unavailable: {type(_be).__name__}", "journal_manager_tail": f"unavailable: {type(_be).__name__}", "nvidia_smi": f"unavailable: {type(_be).__name__}", "probed_model": ""}
            try:
                log_api_event("llamaswap_guard_child_exited_with_bundle", {"returncode": rc, "bundle": _bundle_gc})
            except Exception:
                pass
            ...
            try:
                ...
                if disappeared:
                    ...
                    for model_id in sorted(disappeared):
                        ...
                        try:
                            from llamacpp_stack.cli.crash_bundle import collect_engine_crash_bundle as _collect_unload  # type: ignore

                            _engine_hint_unload = "llama-server"
                            try:
                                _m = next((x for x in catalog if x.model_id == model_id), None)
                                if _m is not None and isinstance(getattr(_m, "server_overrides", None), dict):
                                    _eh = _m.server_overrides.get("engine")
                                    if isinstance(_eh, str) and _eh.strip():
                                        _engine_hint_unload = _eh.strip()
                            except Exception:
                                pass
                            _bundle_unload = _collect_unload(_engine_hint_unload, port=None, pid=None, cmdline=None, returncode=None, timeout_s=8.0)
                        except Exception as _be:
                            _bundle_unload = {"engine": "llama-server", "pid": None, "port": None, "cmdline": f"unavailable: {type(_be).__name__}", "returncode": None, "journal_router_tail": f"unavailable: {type(_be).__name__}", "journal_manager_tail": f"unavailable: {type(_be).__name__}", "nvidia_smi": f"unavailable: {type(_be).__name__}", "probed_model": ""}
                        try:
                            log_api_event("model_unexpected_unload_with_bundle", {"model": model_id, "previous_pid": _bundle_unload.get("pid"), "bundle": _bundle_unload})
                        except Exception:
                            pass
                        log_model_runtime_snapshot(
...
def _tail_text_file(path: Path, lines: int = 50) -> str:
    ...
    try:
        if str(path).endswith(".gz"):
            ...
            data = gzip.open(path, "rt", encoding="utf-8", errors="replace").read().splitlines()  # type: ignore
        else:
...
def _read_log_file_lines(path: Path) -> list[str]:
    try:
        if str(path).endswith(".gz"):
            import gzip

            return gzip.open(path, "rt", encoding="utf-8", errors="replace").read().splitlines()  # type: ignore
        return path.read_text(encoding="utf-8", errors="replace").splitlines()
    except Exception:
        return []


def _rotated_sort_key(path: Path) -> tuple[str, int, str]:
    name = path.name
    # Find date YYYY-MM-DD and optional .partN
    m = re.search(r"\.(\d{4}-\d{2}-\d{2})(?:\.part(\d+))?(?:\.gz)?$", name)
    if m:
        date_str = m.group(1)
        part = int(m.group(2)) if m.group(2) is not None else 0
        return (date_str, part, str(path))
    # Fallback: treat as oldest
    return ("", 0, str(path))


def _glob_rotated_for_base(base: Path) -> list[Path]:
    try:
        # If base is a directory, use it as container with default basename
        if base.exists() and base.is_dir():
            directory = base
            prefix = "api-requests.log"
        else:
            # If base itself looks like a dated file (contains YYYY-MM-DD), treat as direct file
            if re.search(r"\d{4}-\d{2}-\d{2}", base.name):
                # Return the file itself if it exists, plus its .part* siblings
                directory = base.parent
                # For dated file, prefix is base name without .part/.gz but keep date
                # Glob siblings sharing same prefix before .part
                # Use base.name without trailing .partN/.gz as prefix
                base_stem = re.sub(r"(\.part\d+)?(\.gz)?$", "", base.name)
                candidates: list[Path] = []
                if base.exists() and base.is_file():
                    candidates.append(base)
                for p in directory.glob(f"{base_stem}.part*"):
                    if p.is_file():
                        candidates.append(p)
                # Also handle .gz variant
                for p in directory.glob(f"{base_stem}.part*.gz"):
                    if p.is_file() and p not in candidates:
                        candidates.append(p)
                # Deduplicate and sort
                uniq = sorted(set(candidates), key=_rotated_sort_key)
                return uniq
            directory = base.parent
            prefix = base.name
        if not directory.exists():
        ...
        for p in directory.glob(f"{prefix}.*"):
            try:
                if not p.is_file():
                    continue
                suffix = p.name[len(prefix) + 1 :] if p.name.startswith(prefix + ".") else ""
                if not re.match(r"^\d{4}-\d{2}-\d{2}", suffix):
                    continue
                results.append(p)
            except Exception:
                continue
        results.sort(key=_rotated_sort_key)
        return results
    except Exception:
        return []


def _tail_across_files(paths: list[Path], lines: int) -> str:
    existing = [p for p in paths if p.exists() and p.is_file()]
    if not existing:
        return ""
    # Ensure sorted ascending so tail logic works regardless of input order
    existing.sort(key=_rotated_sort_key)
    per_file: list[tuple[Path, list[str]]] = []
    for p in existing:
        per_file.append((p, _read_log_file_lines(p)))
    total = sum(len(d) for _, d in per_file)
    if total == 0:
        return f"No request log entries in {existing[-1]}"
    if total <= lines:
        if len(existing) == 1:
            return "\n".join(per_file[0][1])
        out: list[str] = []
    ...
    for path, data in reversed(per_file):
        if remaining <= 0:
            break
        if not data:
            continue
        take = min(len(data), remaining)
        selected.append((path, data[-take:]))
        remaining -= take
    selected.reverse()
    if len(selected) == 1:
        return "\n".join(selected[0][1])
...
def _candidate_request_log_paths(explicit_path: Path | None = None) -> list[Path]:
    ...
    if explicit_path is not None:
        r = _glob_rotated_for_base(explicit_path)
        if r:
            return sorted(r, key=_rotated_sort_key)
        return [explicit_path]
    seen: set[str] = set()
    rotated_all: list[Path] = []
    legacy: list[Path] = []
    for base in bases:
        r = _glob_rotated_for_base(base)
        if r:
            for p in r:
                key = str(p.resolve()) if p.exists() else str(p)
                if key not in seen:
                    seen.add(key)
                    rotated_all.append(p)
        else:
            if base not in legacy:
                legacy.append(base)
    if rotated_all:
        rotated_all.sort(key=_rotated_sort_key)
        return rotated_all + legacy
    return bases
  ...
  <p class="warn">Nota: un certificado self-signed debe ser confiado manualmente por clientes remotos si quieres evitar avisos TLS.</p>
        ...
        def _proxy_request(self, method: str):
            ...
            try:
                ...
                _bundle_proxy = None
                _bundle_ref_proxy = None
                try:
                    import uuid as _uuid_bundle

                    try:
                        _bundle_ref_proxy = (
                            self.headers.get("X-Request-ID")
                            or self.headers.get("X-Correlation-ID")
                            or _uuid_bundle.uuid4().hex[:8]
                        )
                    except Exception:
                        _bundle_ref_proxy = _uuid_bundle.uuid4().hex[:8]
                    try:
                        from llamacpp_stack.cli.crash_bundle import collect_engine_crash_bundle as _collect_bundle  # type: ignore
                    except Exception:
                        _collect_bundle = None  # type: ignore
                    if _collect_bundle is not None:
                        try:
                            _bundle_proxy = _collect_bundle(
                                "llama-server",
                                port=int(args.public_port) if str(getattr(args, "public_port", "")).isdigit() else None,
                                pid=None,
                                cmdline=None,
                                returncode=None,
                                timeout_s=8.0,
                            ...
                            _bundle_proxy = {"engine": "llama-server", "error": f"unavailable: {type(_be).__name__}", "journal_router_tail": f"unavailable: {type(_be).__name__}", "journal_manager_tail": f"unavailable: {type(_be).__name__}", "nvidia_smi": f"unavailable: {type(_be).__name__}", "probed_model": ""}
                    else:
                        _bundle_proxy = {"engine": "llama-server", "journal_router_tail": "unavailable: ImportError", "journal_manager_tail": "unavailable: ImportError", "nvidia_smi": "unavailable: ImportError", "probed_model": "", "pid": None, "port": None, "cmdline": "", "returncode": None}
                    try:
                        log_api_event(
                            "proxy_error_with_bundle",
                            {"method": method, "path": parsed.path, "error": str(exc)[:1000], "bundle": _bundle_proxy, "bundle_ref": _bundle_ref_proxy},
                        )
                    except Exception:
                        pass
                except Exception:
                ...
                try:
                    ...
                    _dedup_send_json_with_header(
                        self,
                        {"error": _err_short, "engine": _engine_name, "hint": _hint, "bundle_ref": _ref},
                        status=502,
                        dedup_header="miss",
                    )
                except Exception:
...
def show_request_log(args):
    ...
    candidates = _candidate_request_log_paths(explicit_path)
    existing = [p for p in candidates if p.exists() and p.is_file()]
    if existing:
        out = _tail_across_files(existing, lines)
        if out.startswith("-- "):
            print(out)
        else:
            if len(existing) == 1:
                print(f"-- {existing[0]}")
            elif out and "No request log" not in out:
                # Determine if tail came from single newest file but multiple exist
                # In that case _tail_across_files returns without prefix; add prefix for clarity
                # Only when existing has multiple and out is short, add prefix of newest
                newest = sorted(existing, key=_rotated_sort_key)[-1]
                print(f"-- {newest}")
            print(out)
        return 0
    # No rotated files found: check legacy explicit handling for helpful message
    missing = [str(p) for p in candidates]
    print("No request log found. Checked:")
    for path in missing:
        print(f"  {path}")
    print("If the service runs under systemd/root, try:")
    print(f"  sudo {CLI_COMMAND} requests --path {SYSTEM_REQUESTS_LOG_PATH} --lines {lines}")
    return 0
...
def show_logs(args):
    ...
    if existing:
        ...
        for path in candidates:
            print(f"  {path}")
    print('Crash bundles: buscar "<bundle_ref>" o "*_with_bundle" en el log rotado; usar --journal para journal completo')
    print()
...
try:
    from llamacpp_stack.cli.replica import (  # type: ignore
        _calculate_llama_swap_matrix as _sc_calc,
    ...
    pass
try:
    from llamacpp_stack.cli.server_commands import (  # type: ignore
        _append_llama_server_flag as _sc_append,
        build_llama_server_command as _sc_build_llama,
        build_vllm_server_command as _sc_build_vllm,
        get_server_supported_flags as _sc_get_flags,
        normalize_server_overrides as _sc_norm,
        resolve_api_ctx_factor as _sc_api_ctx,
        resolve_llama_server_defaults as _sc_llama_def,
        resolve_request_reasoning_budget as _sc_reasoning,
        resolve_vllm_defaults as _sc_vllm_def,
        server_supports_flag as _sc_supports,
    )
    _append_llama_server_flag = _sc_append  # type: ignore
    build_llama_server_command = _sc_build_llama  # type: ignore
    build_vllm_server_command = _sc_build_vllm  # type: ignore
    get_server_supported_flags = _sc_get_flags  # type: ignore
    normalize_server_overrides = _sc_norm  # type: ignore
    resolve_api_ctx_factor = _sc_api_ctx  # type: ignore
    resolve_llama_server_defaults = _sc_llama_def  # type: ignore
    resolve_request_reasoning_budget = _sc_reasoning  # type: ignore
    resolve_vllm_defaults = _sc_vllm_def  # type: ignore
    server_supports_flag = _sc_supports  # type: ignore
except Exception:
    pass
try:
    from llamacpp_stack.cli.gateway import (  # type: ignore
        DEDUP_STATE as _gw_DEDUP_STATE,
        _DEDUP_STREAM_TEES as _gw_DEDUP_TEES2,
        _dedup_build_fingerprint as _gw_dedup_build2,
        _dedup_extract_principal as _gw_dedup_extract2,
        _dedup_send_cached_response as _gw_send_cached2,
        _dedup_send_json_with_header as _gw_send_json2,
        _dedup_should_bypass as _gw_should2,
        _dedup_streaming_finalize as _gw_finalize2,
        _default_dedup_inflight_config as _gw_default2,
        _normalize_dedup_inflight_config as _gw_norm2,
        _log_chat_stop_without_tools as _gw_log2,
        _tool_call_debug_summary as _gw_tool2,
        resolve_chat_last_response_log_config as _gw_resolve2,
        _normalize_experimental_config as _gw_exp_norm2,
        run_manager_command as _gw_run2,
        manager_hint as _gw_hint2,
        get_public_endpoint_status as _gw_pub2,
    )

    DEDUP_STATE = _gw_DEDUP_STATE  # type: ignore
    _DEDUP_STREAM_TEES = _gw_DEDUP_TEES2  # type: ignore
    _dedup_build_fingerprint = _gw_dedup_build2  # type: ignore
    _dedup_extract_principal = _gw_dedup_extract2  # type: ignore
    _dedup_send_cached_response = _gw_send_cached2  # type: ignore
    _dedup_send_json_with_header = _gw_send_json2  # type: ignore
    _dedup_should_bypass = _gw_should2  # type: ignore
    _dedup_streaming_finalize = _gw_finalize2  # type: ignore
    _default_dedup_inflight_config = _gw_default2  # type: ignore
    _normalize_dedup_inflight_config = _gw_norm2  # type: ignore
    _log_chat_stop_without_tools = _gw_log2  # type: ignore
    _tool_call_debug_summary = _gw_tool2  # type: ignore
    resolve_chat_last_response_log_config = _gw_resolve2  # type: ignore
    _normalize_experimental_config = _gw_exp_norm2  # type: ignore
    run_manager_command = _gw_run2  # type: ignore
    manager_hint = _gw_hint2  # type: ignore
    get_public_endpoint_status = _gw_pub2  # type: ignore
except Exception:
    pass
try:
    from llamacpp_stack.cli.daemon import (  # type: ignore
        _file_signature as _dm_file_sig,
        _prepare_manager_socket_path as _dm_prepare,
        build_info_text as _dm_build_info,
        daemon_mode as _dm_daemon,
        get_heimdall_gateway_version as _dm_version,
        read_install_manifest as _dm_manifest,
        render_heimdall_gateway_banner as _dm_banner,
        show_info as _dm_show_info,
        start_catalog_auto_update_watch as _dm_watch,
        sync_config_from_server_config_for_startup as _dm_sync,
    )

    _file_signature = _dm_file_sig  # type: ignore
    _prepare_manager_socket_path = _dm_prepare  # type: ignore
    build_info_text = _dm_build_info  # type: ignore
    daemon_mode = _dm_daemon  # type: ignore
    get_heimdall_gateway_version = _dm_version  # type: ignore
    read_install_manifest = _dm_manifest  # type: ignore
    render_heimdall_gateway_banner = _dm_banner  # type: ignore
    show_info = _dm_show_info  # type: ignore
    start_catalog_auto_update_watch = _dm_watch  # type: ignore
    sync_config_from_server_config_for_startup = _dm_sync  # type: ignore
except Exception:
    pass
