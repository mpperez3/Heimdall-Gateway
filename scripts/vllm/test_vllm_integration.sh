#!/usr/bin/env bash
# Exhaustive vLLM integration test for llamacpp-stack
set -euo pipefail
...
export LLAMACPP_BACKEND=vllm-beta
echo "DEBUG: LLAMACPP_BACKEND=$LLAMACPP_BACKEND"
...
cleanup() {
    ...
    pkill -f llamacpp-superserver || true
...
mkdir -p /app/logs
mkdir -p /var/lib/llamacpp-superserver/models
mkdir -p /etc/llamacpp-superserver
...
llamacpp-superserver add -hf "$HELPER_MODEL" --skip-ctx --defer-publish
assert_success "Helper model registered"

# 3. Start daemons
echo "[3/10] Starting daemons in background..."
llamacpp-superserver-manager-start > /app/logs/manager.log 2>&1 &
llamacpp-superserver-swap-start > /app/logs/swap.log 2>&1 &
sleep 5

# 4. Verify catalog
echo "[4/10] Verifying catalog..."
llamacpp-superserver list | grep "Qwen2.5-0.5B-Instruct" > /dev/null
assert_success "Catalog listed correctly"

# 5. Load model via vLLM
echo "[5/10] Loading model via vLLM backend..."
MODEL_ID=$(llamacpp-superserver list | grep "Qwen2.5-0.5B-Instruct" | awk '{print $1}' | head -n 1)
llamacpp-superserver run --model-id "$MODEL_ID" --no-chat --ctx-size 512 --float16 --gpu-memory-utilization 0.4 &
sleep 5
...
llamacpp-superserver run -hf "$HELPER_MODEL" -hf "$HELPER_MODEL" --speculative --no-chat --model-id "spec-test" &
sleep 5
...
llamacpp-superserver update --model-id "$MODEL_ID" --description "Validated vLLM Model" --defer-publish
llamacpp-superserver list | grep "Validated vLLM Model" > /dev/null
assert_success "Update command works"

# 10. Final Verification
echo "[10/10] Final verification..."
llamacpp-superserver info | grep "vllm-beta" > /dev/null
