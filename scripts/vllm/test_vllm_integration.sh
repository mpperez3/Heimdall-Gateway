#!/usr/bin/env bash
# Exhaustive vLLM integration test for LLM Server (migrated from Heimdall Gateway)
set -euo pipefail
...
# Ensure we are using vLLM backend (LLM_SERVER_BACKEND canonical, HEIMDALL_GATEWAY_BACKEND legacy fallback)
export LLM_SERVER_BACKEND=vllm-beta
export HEIMDALL_GATEWAY_BACKEND=vllm-beta
echo "DEBUG: LLM_SERVER_BACKEND=$LLM_SERVER_BACKEND (legacy HEIMDALL_GATEWAY_BACKEND=$HEIMDALL_GATEWAY_BACKEND)"
...
mkdir -p /app/logs
mkdir -p /var/lib/llm-server/models
mkdir -p /etc/llm-server
...
llm-server add -hf "$HELPER_MODEL" --skip-ctx --defer-publish
assert_success "Helper model registered"

# 3. Start daemons
echo "[3/10] Starting daemons in background..."
llm-server-manager-start > /app/logs/manager.log 2>&1 &
llm-server-router-start > /app/logs/swap.log 2>&1 &
sleep 5

# 4. Verify catalog
echo "[4/10] Verifying catalog..."
llm-server list | grep "Qwen2.5-0.5B-Instruct" > /dev/null
assert_success "Catalog listed correctly"

# 5. Load model via vLLM
echo "[5/10] Loading model via vLLM backend..."
MODEL_ID=$(llm-server list | grep "Qwen2.5-0.5B-Instruct" | awk '{print $1}' | head -n 1)
llm-server run --model-id "$MODEL_ID" --no-chat --ctx-size 512 --float16 --gpu-memory-utilization 0.4 &
sleep 5
...
llm-server run -hf "$HELPER_MODEL" -hf "$HELPER_MODEL" --speculative --no-chat --model-id "spec-test" &
sleep 5
...
llm-server update --model-id "$MODEL_ID" --description "Validated vLLM Model" --defer-publish
llm-server list | grep "Validated vLLM Model" > /dev/null
assert_success "Update command works"

# 10. Final Verification
echo "[10/10] Final verification..."
llm-server info | grep "vllm-beta" > /dev/null
